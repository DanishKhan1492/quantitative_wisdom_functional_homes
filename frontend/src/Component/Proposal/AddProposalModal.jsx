...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    onClose();
    onSubmit(proposalData); // Refresh parent list
  } catch (error) {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await getAllProducts(0, 10, productSearch);
  ...
  const handleProductSearch = async (searchTerm) => {
    try {
      const response = await getAllProducts(0, 10, searchTerm);
      setProducts(response.content);
    } catch (error) {
                ...
                <div className="space-y-8">
                  <div className="grid grid-cols-3 gap-6">
                   ...
                   hover:border-blue-400"
                    ...
                    <div className="group">
                      <label className="block text-md font-medium text-black mb-2 group-hover:text-blue-400 transition-colors duration-200">
                        Appartment Type <span className="text-red-500">*</span>
                      </label>
                      <select
                        name="apartmentTypeId"
                        value={formValues.apartmentTypeId}
                        onChange={handleChange}
                      ...
                      >
                        ...
                        {apartmenttype.map((type) => (
                          <option
                            key={type.apartmentId}
                            value={type.apartmentId}
                          >
                            {type.name}
                          </option>
                        ))}
...
//      apartmentTypeId: formValues.apartmentTypeId,
...
//    try {
//      if (isEditMode) {
//        await updateProposal(proposalToEdit.id, proposalData);
//        toast.success("Proposal updated successfully");
//      } else {
//        await createProposal(proposalData);
...
//      }
//      onClose();
//      // Refresh proposal list in parent component
//    } catch (error) {
//      toast.error(`Failed to ${isEditMode ? "update" : "create"} proposal`);
//      console.error("Submission error:", error);
//    }
//  };
// const [selectedProducts, setSelectedProducts] = useState([]);
